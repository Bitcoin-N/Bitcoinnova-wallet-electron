...
const ERROR_WALLETLAUNCH = 'Failed to start Bitcoinnova-service. Set the path to Bitcoinnova-service properly in the settings tab.';
...
const DEFAULT_WALLET_EXT = 'wallet';
        ...
        settings.get('service_bin'), [
            ...
            if(error){
                ...
                if(stdout && stdout.length && stdout.indexOf('E') !== -1){
                    ...
                    let wa = trimmed.substring(trimmed.indexOf('E'), trimmed.length);
...
function doRunService(filePath, password, scanHeight, onError, onSuccess) {
    ...
    serviceProcess.on('close', function (code, signal) {
        log.debug(`Bitcoinnova-service terminated by ${signal}, code: ${code}`);
        serviceProcess = null;
    });
    ...
    if (isRunning()) {
        ...
        log.debug('Bitcoinnova-service not running');
